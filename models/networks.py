...
def define_G(opt):
    ...
    if gpu_ids is not None:
        ...
        #netG = nn.DataParallel(netG) #original

        # infiniband network, cu01 --> 10.0.0.1;cu02 --> 10.0.0.2
        dist.init_process_group(backend='nccl',
                                init_method="tcp://"+opt['master_ip_port'], 
        ...
        netG.cuda()
        netG = nn.parallel.DistributedDataParallel(netG, device_ids=[int(opt['gpu_ids'])])
        print('graph G distributed on node {} with gpu {}'.format(opt['node_index'], opt['gpu_ids']))
    else:
        assert torch.cuda.is_available()
        dist.init_process_group(backend='nccl',
                                init_method="tcp://"+opt['master_ip_port'],
        ...
        print(netG.cuda())
        netG = nn.parallel.DistributedDataParallel(netG).cuda()
    return netG
...
def define_D(opt):
    ...
    print('init d_graph successed')
    if gpu_ids is not None:
        #netD = nn.DataParallel(netD)
        print(netD.cuda())
        netD = nn.parallel.DistributedDataParallel(netD,device_ids=[int(opt['gpu_ids'])])
    return netD
...
def define_F(opt, use_bn=False):
    ...
    if gpu_ids is not None:
        netF.cuda()
        netF = nn.parallel.DistributedDataParallel(netF,device_ids=[int(opt['gpu_ids'])])
